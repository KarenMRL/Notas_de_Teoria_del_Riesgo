...
\begin{eqnarray*} 
\mathbb{E}[Y_{L}^{k}]&=& \mathbb{E}[\mathbb{E}[Y_{L}^{k}|Y_{L}]]\\
\end{eqnarray*}
\begin{eqnarray*} 
=\mathbb{E}[Y_{L}^{k}]\mathbb{P}[Y_{L} \equiv 0]+\mathbb{E}[Y_{L}^{k}|Y_{L}>0]\mathbb{P}[Y_{L}>0]\\
\end{eqnarray*}
...
\begin{equation*}
        \mathbb E [g(Y_{L})] =\\
         g(0)[F_{\chi}(\frac{d}{1+r})]+ \int_{0}^{\alpha (u-d)} g(t) \frac{f_{\chi}(\frac{1+d\alpha}{\alpha(1+r)})}{\alpha(1+r)} dt + 
...
\begin{equation*}
        ...
        \text{Obtengamos $q_{\chi}(0.5)$ la medianda de $\chi$}\\
        0.5 = F_{\chi}(\chi_{0}) = \frac{\chi_{0}-100}{100} \Longleftrightarrow \chi_{0} = 150 = q_{\chi}(0.5)\\
        \longrightarrow q_{Y_{L}}(0.5) = 
        max\{0.75(min\{q_{\chi}(0.5)(1.1),180\}-120),0\}\\
...
\begin{equation*}
        ...
        \longrightarrow q_{Y_{L}}(0.9) = 
        max\{0.75(min\{q_{\chi}(0.9)(1.1),180\}-120),0\}\\
